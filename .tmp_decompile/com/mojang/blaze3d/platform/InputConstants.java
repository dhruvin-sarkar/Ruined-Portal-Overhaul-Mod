...
public class InputConstants {
    ...
    public static Key getKey(KeyEvent keyEvent) {
        if (keyEvent.key() == -1) {
            return Type.SCANCODE.getOrCreate(keyEvent.scancode());
        }
        return Type.KEYSYM.getOrCreate(keyEvent.key());
    }
    ...
    public static boolean isKeyDown(Window window, int i) {
        return GLFW.glfwGetKey((long)window.handle(), (int)i) == 1;
    }

    public static void setupKeyboardCallbacks(Window window, GLFWKeyCallbackI gLFWKeyCallbackI, GLFWCharModsCallbackI gLFWCharModsCallbackI) {
        GLFW.glfwSetKeyCallback((long)window.handle(), (GLFWKeyCallbackI)gLFWKeyCallbackI);
        GLFW.glfwSetCharModsCallback((long)window.handle(), (GLFWCharModsCallbackI)gLFWCharModsCallbackI);
    }

    public static void setupMouseCallbacks(Window window, GLFWCursorPosCallbackI gLFWCursorPosCallbackI, GLFWMouseButtonCallbackI gLFWMouseButtonCallbackI, GLFWScrollCallbackI gLFWScrollCallbackI, GLFWDropCallbackI gLFWDropCallbackI) {
        GLFW.glfwSetCursorPosCallback((long)window.handle(), (GLFWCursorPosCallbackI)gLFWCursorPosCallbackI);
        GLFW.glfwSetMouseButtonCallback((long)window.handle(), (GLFWMouseButtonCallbackI)gLFWMouseButtonCallbackI);
        GLFW.glfwSetScrollCallback((long)window.handle(), (GLFWScrollCallbackI)gLFWScrollCallbackI);
        GLFW.glfwSetDropCallback((long)window.handle(), (GLFWDropCallbackI)gLFWDropCallbackI);
    }

    public static void grabOrReleaseMouse(Window window, int i, double d, double e) {
        GLFW.glfwSetCursorPos((long)window.handle(), (double)d, (double)e);
        GLFW.glfwSetInputMode((long)window.handle(), (int)208897, (int)i);
    }
    ...
    public static void updateRawMouseInput(Window window, boolean bl) {
        if (InputConstants.isRawMouseInputSupported()) {
            GLFW.glfwSetInputMode((long)window.handle(), (int)GLFW_RAW_MOUSE_MOTION, (int)(bl ? 1 : 0));
        }
    }
    ...
    public static final class Key {
        ...
        Key(String string, Type type, int i) {
            ...
            this.type = type;
            this.value = i;
            this.displayName = Suppliers.memoize(() -> type.displayTextSupplier.apply(i, string));
