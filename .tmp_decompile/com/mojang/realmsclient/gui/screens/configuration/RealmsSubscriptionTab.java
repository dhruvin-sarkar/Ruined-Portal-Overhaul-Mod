...
implements RealmsConfigurationTab {
    ...
    RealmsSubscriptionTab(RealmsConfigureWorldScreen realmsConfigureWorldScreen, Minecraft minecraft, RealmsServer realmsServer) {
        ...
        rowHelper.addChild(new StringWidget(200, font.lineHeight, SUBSCRIPTION_START_LABEL, font));
        this.startDateWidget = rowHelper.addChild(new StringWidget(200, font.lineHeight, this.startDate, font));
        rowHelper.addChild(SpacerElement.height(2));
        this.daysLeftLabelWidget = rowHelper.addChild(new StringWidget(200, font.lineHeight, TIME_LEFT_LABEL, font));
        this.daysLeftWidget = rowHelper.addChild(new StringWidget(200, font.lineHeight, this.daysLeft, font));
        rowHelper.addChild(SpacerElement.height(2));
        ...
        rowHelper.addChild(SpacerElement.height(2));
        this.subscriptionInfo = rowHelper.addChild(FocusableTextWidget.builder(Component.empty(), font).maxWidth(200).build(), LayoutSettings.defaults().alignHorizontallyCenter());
    ...
    private static Component localPresentation(Instant instant) {
        String string = ZonedDateTime.ofInstant(instant, ZoneId.systemDefault()).format(Util.localizedDateFormatter(FormatStyle.MEDIUM));
    ...
    private Component daysLeftPresentation(int i) {
        ...
        if (bl2) {
            return Component.translatable("mco.configure.world.subscription.remaining.days", k).withStyle(ChatFormatting.GRAY);
        }
