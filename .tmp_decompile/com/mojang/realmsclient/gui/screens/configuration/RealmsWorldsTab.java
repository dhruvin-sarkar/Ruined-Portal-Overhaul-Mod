...
implements RealmsConfigurationTab {
    ...
    RealmsWorldsTab(RealmsConfigureWorldScreen realmsConfigureWorldScreen, Minecraft minecraft, RealmsServer realmsServer) {
        ...
        this.optionsButton = rowHelper3.addChild(Button.builder(Component.translatable("mco.configure.world.buttons.options"), button -> minecraft.setScreen(new RealmsSlotOptionsScreen(realmsConfigureWorldScreen, realmsServer.slots.get(realmsServer.activeSlot).copy(), realmsServer.worldType, realmsServer.activeSlot))).bounds(0, 0, 150, 20).build());
        this.backupButton = rowHelper3.addChild(Button.builder(Component.translatable("mco.configure.world.backup"), button -> minecraft.setScreen(new RealmsBackupScreen(realmsConfigureWorldScreen, realmsServer.copy(), realmsServer.activeSlot))).bounds(0, 0, 150, 20).build());
    ...
    private void switchToMinigame() {
        RealmsSelectWorldTemplateScreen realmsSelectWorldTemplateScreen = new RealmsSelectWorldTemplateScreen(Component.translatable("mco.template.title.minigame"), this::templateSelectionCallback, RealmsServer.WorldType.MINIGAME, null, List.of((Object)Component.translatable("mco.minigame.world.info.line1").withColor(-4539718), (Object)Component.translatable("mco.minigame.world.info.line2").withColor(-4539718)));
    ...
    private void switchToFullSlot(int i, RealmsServer realmsServer) {
        this.minecraft.setScreen(RealmsPopups.infoPopupScreen(this.configurationScreen, Component.translatable("mco.configure.world.slot.switch.question.line1"), popupScreen -> {
            RealmsConfigureWorldScreen realmsConfigureWorldScreen = this.configurationScreen.getNewScreen();
            this.configurationScreen.stateChanged();
            this.minecraft.setScreen(new RealmsLongRunningMcoTaskScreen(realmsConfigureWorldScreen, new SwitchSlotTask(realmsServer.id, i, () -> this.minecraft.execute(() -> this.minecraft.setScreen(realmsConfigureWorldScreen)))));
        }));
    }
    ...
    private void switchToEmptySlot(int i, RealmsServer realmsServer) {
        this.minecraft.setScreen(RealmsPopups.infoPopupScreen(this.configurationScreen, Component.translatable("mco.configure.world.slot.switch.question.line1"), popupScreen -> {
            this.configurationScreen.stateChanged();
            RealmsResetWorldScreen realmsResetWorldScreen = RealmsResetWorldScreen.forEmptySlot(this.configurationScreen, i, realmsServer, () -> this.minecraft.execute(() -> this.minecraft.setScreen(this.configurationScreen.getNewScreen())));
