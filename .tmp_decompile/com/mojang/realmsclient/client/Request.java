...
public abstract class Request<T extends Request<T>> {
    ...
    public static void cookie(HttpURLConnection httpURLConnection, String string, String string2) {
        String string3 = httpURLConnection.getRequestProperty(COOKIE_KEY);
        if (string3 == null) {
            httpURLConnection.setRequestProperty(COOKIE_KEY, string + "=" + string2);
        } else {
            httpURLConnection.setRequestProperty(COOKIE_KEY, string3 + ";" + string + "=" + string2);
        }
    }

    public void addSnapshotHeader(boolean bl) {
        this.connection.addRequestProperty(IS_SNAPSHOT_KEY, String.valueOf(bl));
    }
