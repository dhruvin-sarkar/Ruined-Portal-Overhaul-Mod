...
public class PerfCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("perf").requires(Commands.hasPermission(Commands.LEVEL_OWNERS))).then(Commands.literal("start").executes(commandContext -> PerfCommand.startProfilingDedicatedServer((CommandSourceStack)commandContext.getSource())))).then(Commands.literal("stop").executes(commandContext -> PerfCommand.stopProfilingDedicatedServer((CommandSourceStack)commandContext.getSource()))));
    }
    ...
    private static void saveResults(CommandSourceStack commandSourceStack, Path path, MinecraftServer minecraftServer) {
        String string2;
        String string = String.format(Locale.ROOT, "%s-%s-%s", Util.getFilenameFormattedDateTime(), minecraftServer.getWorldData().getLevelName(), SharedConstants.getCurrentVersion().id());
        try {
