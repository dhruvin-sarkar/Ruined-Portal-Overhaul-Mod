...
public record Schema<T>(Optional<URI> reference, List<String> type, Optional<Schema<?>> items, Map<String, Schema<?>> properties, List<String> enumValues, Codec<T> codec) {
    public static final Codec<? extends Schema<?>> CODEC = Codec.recursive((String)"Schema", codec -> RecordCodecBuilder.create(instance -> instance.group((App)ReferenceUtil.REFERENCE_CODEC.optionalFieldOf("$ref").forGetter(Schema::reference), (App)ExtraCodecs.compactListCodec(Codec.STRING).optionalFieldOf("type", (Object)List.of()).forGetter(Schema::type), (App)codec.optionalFieldOf("items").forGetter(Schema::items), (App)Codec.unboundedMap((Codec)Codec.STRING, (Codec)codec).optionalFieldOf("properties", (Object)Map.of()).forGetter(Schema::properties), (App)Codec.STRING.listOf().optionalFieldOf("enum", (Object)List.of()).forGetter(Schema::enumValues)).apply((Applicative)instance, (optional, list, optional2, map, list2) -> null))).validate(schema -> {
        if (schema == null) {
    ...
    public static final Schema<Either<Boolean, Integer>> BOOL_OR_INT_SCHEMA = Schema.ofTypes(List.of((Object)"boolean", (Object)"integer"), Codec.either((Codec)Codec.BOOL, (Codec)Codec.INT));
    ...
    public Schema<T> info() {
        return new Schema<T>(this.reference, this.type, this.items.map(Schema::info), this.properties.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> ((Schema)((Object)((Object)entry.getValue()))).info())), this.enumValues, this.codec);
    }

    private static <T> SchemaComponent<T> registerSchema(String string, Schema<T> schema) {
        SchemaComponent<T> schemaComponent = new SchemaComponent<T>(string, ReferenceUtil.createLocalReference(string), schema);
    ...
    public static <T> Schema<T> ofRef(URI uRI, Codec<T> codec) {
        return new Schema<T>(Optional.of(uRI), List.of(), Optional.empty(), Map.of(), List.of(), codec);
    }

    public static <T> Schema<T> ofType(String string, Codec<T> codec) {
        return Schema.ofTypes(List.of((Object)string), codec);
    }

    public static <T> Schema<T> ofTypes(List<String> list, Codec<T> codec) {
        return new Schema<T>(Optional.empty(), list, Optional.empty(), Map.of(), List.of(), codec);
    }

    public static <E extends Enum<E>> Schema<E> ofEnum(Supplier<E[]> supplier) {
        return Schema.ofEnum(supplier, StringRepresentable.fromEnum(supplier));
    }

    public static <E extends Enum<E>> Schema<E> ofEnum(Supplier<E[]> supplier, Codec<E> codec) {
        List list = Stream.of((Enum[])supplier.get()).map(object -> ((StringRepresentable)object).getSerializedName()).toList();
        return Schema.ofEnum(list, codec);
    }

    public static <T> Schema<T> ofEnum(List<String> list, Codec<T> codec) {
        return new Schema<T>(Optional.empty(), List.of((Object)"string"), Optional.empty(), Map.of(), list, codec);
    }

    public static <T> Schema<List<T>> arrayOf(Schema<?> schema, Codec<T> codec) {
        return new Schema<List<T>>(Optional.empty(), List.of((Object)"array"), Optional.of(schema), Map.of(), List.of(), codec.listOf());
    }

    public static <T> Schema<T> record(Codec<T> codec) {
        return new Schema<T>(Optional.empty(), List.of((Object)"object"), Optional.empty(), Map.of(), List.of(), codec);
    }

    private static <T> Schema<T> record(Map<String, Schema<?>> map, Codec<T> codec) {
        return new Schema<T>(Optional.empty(), List.of((Object)"object"), Optional.empty(), map, List.of(), codec);
    }

    public Schema<T> withField(String string, Schema<?> schema) {
        HashMap hashMap = new HashMap(this.properties);
        hashMap.put(string, schema);
        return Schema.record(hashMap, this.codec);
    }
