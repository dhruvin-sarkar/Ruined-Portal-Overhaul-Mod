...
implements DataProvider {
    ...
    public <T> PackMetadataGenerator add(MetadataSectionType<T> metadataSectionType, T object) {
        this.elements.put(metadataSectionType.name(), () -> ((JsonElement)metadataSectionType.codec().encodeStart((DynamicOps)JsonOps.INSTANCE, object).getOrThrow(IllegalArgumentException::new)).getAsJsonObject());
        return this;
    }
    ...
    public static PackMetadataGenerator forFeaturePack(PackOutput packOutput, Component component) {
        return new PackMetadataGenerator(packOutput).add(PackMetadataSection.SERVER_TYPE, new PackMetadataSection(component, DetectedVersion.BUILT_IN.packVersion(PackType.SERVER_DATA).minorRange()));
    }
