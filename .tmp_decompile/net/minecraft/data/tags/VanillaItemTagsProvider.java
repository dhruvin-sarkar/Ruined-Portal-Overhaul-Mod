...
extends IntrinsicHolderTagsProvider<Item> {
    ...
    protected void addTags(HolderLookup.Provider provider) {
        ...
        this.tag(ItemTags.TRIM_MATERIALS).addAll(provider.lookupOrThrow(Registries.ITEM).listElements().filter(reference -> ((Item)reference.value()).components().has(DataComponents.PROVIDES_TRIM_MATERIAL)).sorted(Comparator.comparing(reference -> reference.key().identifier())).map(Holder.Reference::value));
