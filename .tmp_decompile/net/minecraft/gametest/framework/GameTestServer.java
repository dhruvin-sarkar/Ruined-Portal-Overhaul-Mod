...
extends MinecraftServer {
    ...
    private GameTestServer(Thread thread, LevelStorageSource.LevelStorageAccess levelStorageAccess, PackRepository packRepository, WorldStem worldStem, Optional<String> optional, boolean bl) {
        super(thread, levelStorageAccess, packRepository, worldStem, Proxy.NO_PROXY, DataFixers.getDataFixer(), NO_SERVICES, LoggingLevelLoadListener.forDedicatedServer());
    ...
    private List<GameTestBatch> evaluateTestsToRun(ServerLevel serverLevel) {
        ...
        List collection;
        HolderLookup.RegistryLookup registry = serverLevel.registryAccess().lookupOrThrow(Registries.TEST_INSTANCE);
        if (this.testSelection.isPresent()) {
            collection = GameTestServer.getTestsForSelection(serverLevel.registryAccess(), this.testSelection.get()).filter(reference -> !((GameTestInstance)reference.value()).manualOnly()).toList();
            if (this.verify) {
                ...
                LOGGER.info("Will run tests matching {} ({} tests)", (Object)this.testSelection.get(), (Object)collection.size());
            }
        } else {
    ...
    private void startTests(ServerLevel serverLevel) {
        ...
        List<GameTestInfo> collection = gameTestRunner.getTestInfos();
    ...
    implements UserNameToIdResolver {
        ...
        public Optional<NameAndId> get(String string) {
            return this.savedIds.stream().filter(nameAndId -> nameAndId.name().equals(string)).findFirst().or(() -> Optional.of(NameAndId.createOffline(string)));
        }
