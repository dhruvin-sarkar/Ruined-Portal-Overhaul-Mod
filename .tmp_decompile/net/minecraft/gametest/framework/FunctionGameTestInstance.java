...
extends GameTestInstance {
    ...
    public void run(GameTestHelper gameTestHelper) {
        gameTestHelper.getLevel().registryAccess().get(this.function).map(Holder.Reference::value).orElseThrow(() -> new IllegalStateException("Trying to access missing test function: " + String.valueOf(this.function.identifier()))).accept(gameTestHelper);
    }
