...
implements ProfileResults {
    ...
    private Map<String, CounterCollector> getCounterValues() {
        ...
        this.entries.forEach((string, profilerPathEntry) -> {
            ...
            if (!object2LongMap.isEmpty()) {
                ...
                object2LongMap.forEach((string2, l) -> map.computeIfAbsent(string2, string -> new CounterCollector()).addValue(list.iterator(), l));
            }
        });
        return map;
    }
    ...
    private void appendProfilerResults(int i, String string2, StringBuilder stringBuilder) {
        ...
        object2LongMap.forEach((string, l) -> FilledProfileResults.indentLine(stringBuilder, i).append('#').append((String)string).append(' ').append(l).append('/').append(l / (long)this.tickDuration).append('\n'));
        if (list.size() < 3) {
            return;
        }
