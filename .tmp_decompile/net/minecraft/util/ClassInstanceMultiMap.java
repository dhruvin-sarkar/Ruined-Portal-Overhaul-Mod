...
extends AbstractCollection<T> {
    ...
    public <S> Collection<S> find(Class<S> class_2) {
        if (!this.baseClass.isAssignableFrom(class_2)) {
            throw new IllegalArgumentException("Don't know how to search for " + String.valueOf(class_2));
        }
        List list = this.byClass.computeIfAbsent(class_2, class_ -> this.allInstances.stream().filter(class_::isInstance).collect(Util.toMutableList()));
