...
public class DebugSubscriptions<T> {
    ...
    private static DebugSubscription<?> registerSimple(String string) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(string), new DebugSubscription(null));
    }

    private static <T> DebugSubscription<T> registerWithValue(String string, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(string), new DebugSubscription<T>(streamCodec));
    }

    private static <T> DebugSubscription<T> registerTemporaryValue(String string, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec, int i) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(string), new DebugSubscription<T>(streamCodec, i));
    }
}
