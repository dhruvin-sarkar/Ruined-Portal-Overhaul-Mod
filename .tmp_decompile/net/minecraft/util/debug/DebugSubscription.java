...
public class DebugSubscription<T> {
    ...
    public Update<T> packUpdate(@Nullable T object) {
        return new Update<T>(this, Optional.ofNullable(object));
    }
    ...
    public Event<T> packEvent(T object) {
        return new Event<T>(this, object);
    }
    ...
    public record Update<T>(DebugSubscription<T> subscription, Optional<T> value) {
        ...
        private static <T> StreamCodec<? super RegistryFriendlyByteBuf, Update<T>> streamCodec(DebugSubscription<T> debugSubscription) {
            return ByteBufCodecs.optional(Objects.requireNonNull(debugSubscription.valueStreamCodec)).map(optional -> new Update(debugSubscription, optional), Update::value);
        }
    }
    ...
    public record Event<T>(DebugSubscription<T> subscription, T value) {
        ...
        private static <T> StreamCodec<? super RegistryFriendlyByteBuf, Event<T>> streamCodec(DebugSubscription<T> debugSubscription) {
            return Objects.requireNonNull(debugSubscription.valueStreamCodec).map(object -> new Event<Object>(debugSubscription, object), Event::value);
        }
    }
}
