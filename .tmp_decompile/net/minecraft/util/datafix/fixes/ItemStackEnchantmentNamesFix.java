...
extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> dynamic2) {
        Optional optional = dynamic2.get("ench").asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(arg_0 -> dynamic2.createList(arg_0)).result();
        if (optional.isPresent()) {
            dynamic2 = dynamic2.remove("ench").set("Enchantments", (Dynamic)optional.get());
        }
        return dynamic2.update("StoredEnchantments", dynamic -> (Dynamic)DataFixUtils.orElse((Optional)dynamic.asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(arg_0 -> ((Dynamic)dynamic).createList(arg_0)).result(), (Object)dynamic));
    }
}
