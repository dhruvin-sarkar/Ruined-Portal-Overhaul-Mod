...
extends DataFix {
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        ...
        return typed.update(type.finder(), type2, either -> {
            int i = (Integer)either.map(pair -> (Integer)((Either)pair.getSecond()).map(integer -> integer, EntityBlockStateFix::getBlockId), unit -> {
            ...
            return Either.left((Object)Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(i << 4 | j)));
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String string, String string2, String string3) {
        Tag.TagType type = DSL.field((String)string, (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString())));
        Tag.TagType type2 = DSL.field((String)string3, (Type)DSL.named((String)References.BLOCK_STATE.typeName(), (Type)DSL.remainderType()));
        Dynamic dynamic = (Dynamic)typed.getOrCreate(DSL.remainderFinder());
        return typed.update(type.finder(), (Type)type2, pair -> {
            int i = (Integer)((Either)pair.getSecond()).map(integer -> integer, EntityBlockStateFix::getBlockId);
            int j = dynamic.get(string2).asInt(0) & 0xF;
            return Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(i << 4 | j));
