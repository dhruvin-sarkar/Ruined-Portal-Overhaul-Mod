...
extends DataFix {
    ...
    private static boolean isZeroList(OptionalDynamic<?> optionalDynamic, int i) {
        return optionalDynamic.flatMap(arg_0 -> FLOAT_LIST_CODEC.parse(arg_0)).map(list -> list.size() == i && list.stream().allMatch(float_ -> float_.floatValue() == 0.0f)).result().orElse(false);
    }
}
