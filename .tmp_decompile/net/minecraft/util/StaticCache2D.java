...
public class StaticCache2D<T> {
    ...
    public static <T> StaticCache2D<T> create(int i, int j, int k, Initializer<T> initializer) {
        int l = i - k;
        int m = j - k;
        int n = 2 * k + 1;
        return new StaticCache2D<T>(l, m, n, n, initializer);
    }

    private StaticCache2D(int i, int j, int k, int l, Initializer<T> initializer) {
        this.minX = i;
        this.minZ = j;
        this.sizeX = k;
        this.sizeZ = l;
        ...
        for (int m = i; m < i + k; ++m) {
            for (int n = j; n < j + l; ++n) {
                this.cache[this.getIndex((int)m, (int)n)] = initializer.get(m, n);
            }
        }
    }
