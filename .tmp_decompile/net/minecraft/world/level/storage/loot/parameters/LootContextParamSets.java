...
public class LootContextParamSets {
    ...
    public static final Codec<ContextKeySet> CODEC = Identifier.CODEC.comapFlatMap(identifier -> Optional.ofNullable((ContextKeySet)REGISTRY.get(identifier)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "No parameter set exists with id: '" + String.valueOf(identifier) + "'")), arg_0 -> REGISTRY.inverse().get(arg_0));
    ...
    private static ContextKeySet register(String string, Consumer<ContextKeySet.Builder> consumer) {
        ...
        Identifier identifier = Identifier.withDefaultNamespace(string);
        ContextKeySet contextKeySet2 = (ContextKeySet)REGISTRY.put((Object)identifier, (Object)contextKeySet);
