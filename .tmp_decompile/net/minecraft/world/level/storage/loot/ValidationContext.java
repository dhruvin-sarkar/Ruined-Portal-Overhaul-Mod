...
public class ValidationContext {
    ...
    public ValidationContext enterElement(ProblemReporter.PathElement pathElement, ResourceKey<?> resourceKey) {
        ImmutableSet set = ImmutableSet.builder().addAll(this.visitedElements).add(resourceKey).build();
        return new ValidationContext(this.reporter.forChild(pathElement), this.contextKeySet, this.resolver, (Set<ResourceKey<?>>)set);
    }
    ...
    public void validateContextUsage(LootContextUser lootContextUser) {
        ...
        if (!set2.isEmpty()) {
            this.reporter.report(new ParametersNotProvidedProblem((Set<ContextKey<?>>)set2));
        }
    }
