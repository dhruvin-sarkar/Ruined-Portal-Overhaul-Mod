...
public class GameRules {
    ...
    private static GameRule<Boolean> registerBoolean(String string, GameRuleCategory gameRuleCategory, boolean bl) {
        return GameRules.register(string, gameRuleCategory, GameRuleType.BOOL, BoolArgumentType.bool(), Codec.BOOL, bl, FeatureFlagSet.of(), GameRuleTypeVisitor::visitBoolean, boolean_ -> boolean_ != false ? 1 : 0);
    }

    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int i, int j) {
        return GameRules.registerInteger(string, gameRuleCategory, i, j, Integer.MAX_VALUE, FeatureFlagSet.of());
    }

    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int i, int j, int k) {
        return GameRules.registerInteger(string, gameRuleCategory, i, j, k, FeatureFlagSet.of());
    }
    ...
    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int i, int j, int k, FeatureFlagSet featureFlagSet) {
        return GameRules.register(string, gameRuleCategory, GameRuleType.INT, IntegerArgumentType.integer((int)j, (int)k), Codec.intRange((int)j, (int)k), i, featureFlagSet, GameRuleTypeVisitor::visitInteger, integer -> integer);
    }

    private static <T> GameRule<T> register(String string, GameRuleCategory gameRuleCategory, GameRuleType gameRuleType, ArgumentType<T> argumentType, Codec<T> codec, T object, FeatureFlagSet featureFlagSet, VisitorCaller<T> visitorCaller, ToIntFunction<T> toIntFunction) {
        return Registry.register(BuiltInRegistries.GAME_RULE, string, new GameRule<T>(gameRuleCategory, gameRuleType, argumentType, visitorCaller, codec, toIntFunction, object, featureFlagSet));
    }
