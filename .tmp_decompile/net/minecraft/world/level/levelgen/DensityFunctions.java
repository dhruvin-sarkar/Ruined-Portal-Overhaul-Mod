...
public final class DensityFunctions {
    ...
    public static DensityFunction findTopSurface(DensityFunction densityFunction, DensityFunction densityFunction2, int i, int j) {
        return new FindTopSurface(densityFunction, densityFunction2, i, j);
    }
    ...
    protected record Mapped(Type type, DensityFunction input, double minValue, double maxValue) implements PureTransformer
    {
        public static Mapped create(Type type, DensityFunction densityFunction) {
            ...
            double f = Mapped.transform(type, d);
            double g = Mapped.transform(type, e);
            if (type == Type.INVERT) {
                if (d < 0.0 && e > 0.0) {
                    return new Mapped(type, densityFunction, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
                }
                return new Mapped(type, densityFunction, g, f);
            }
            if (type == Type.ABS || type == Type.SQUARE) {
                return new Mapped(type, densityFunction, Math.max(0.0, d), Math.max(f, g));
            }
            return new Mapped(type, densityFunction, f, g);
        }
    ...
    extends DensityFunction {
        ...
        public static TwoArgumentSimpleFunction create(Type type, DensityFunction densityFunction, DensityFunction densityFunction2) {
            ...
            if (type == Type.MIN || type == Type.MAX) {
                ...
                boolean bl3 = bl2 = e >= f;
                if (bl || bl2) {
                    LOGGER.warn("Creating a {} function between two non-overlapping inputs: {} and {}", new Object[]{type, densityFunction, densityFunction2});
                }
            }
    ...
    {
        private static final MapCodec<FindTopSurface> DATA_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)DensityFunction.HOLDER_HELPER_CODEC.fieldOf("density").forGetter(FindTopSurface::density), (App)DensityFunction.HOLDER_HELPER_CODEC.fieldOf("upper_bound").forGetter(FindTopSurface::upperBound), (App)Codec.intRange((int)(DimensionType.MIN_Y * 2), (int)(DimensionType.MAX_Y * 2)).fieldOf("lower_bound").forGetter(FindTopSurface::lowerBound), (App)ExtraCodecs.POSITIVE_INT.fieldOf("cell_height").forGetter(FindTopSurface::cellHeight)).apply((Applicative)instance, FindTopSurface::new));
        public static final KeyDispatchDataCodec<FindTopSurface> CODEC = DensityFunctions.makeCodec(DATA_CODEC);

        @Override
        public double compute(DensityFunction.FunctionContext functionContext) {
            int i = Mth.floor(this.upperBound.compute(functionContext) / (double)this.cellHeight) * this.cellHeight;
            if (i <= this.lowerBound) {
                return this.lowerBound;
            }
            for (int j = i; j >= this.lowerBound; j -= this.cellHeight) {
                DensityFunction.SinglePointContext singlePointContext = new DensityFunction.SinglePointContext(functionContext.blockX(), j, functionContext.blockZ());
