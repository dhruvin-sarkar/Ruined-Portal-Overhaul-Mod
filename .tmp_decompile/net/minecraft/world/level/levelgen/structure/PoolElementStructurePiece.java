...
extends StructurePiece {
    ...
    public PoolElementStructurePiece(StructureTemplateManager structureTemplateManager, StructurePoolElement structurePoolElement, BlockPos blockPos, int i, Rotation rotation, BoundingBox boundingBox, LiquidSettings liquidSettings) {
    ...
    public PoolElementStructurePiece(StructurePieceSerializationContext structurePieceSerializationContext, CompoundTag compoundTag) {
        ...
        RegistryOps<Tag> dynamicOps = structurePieceSerializationContext.registryAccess().createSerializationContext(NbtOps.INSTANCE);
        this.element = compoundTag.read("pool_element", StructurePoolElement.CODEC, dynamicOps).orElseThrow(() -> new IllegalStateException("Invalid pool element found"));
        this.rotation = (Rotation)compoundTag.read("rotation", Rotation.LEGACY_CODEC).orElseThrow();
    ...
    protected void addAdditionalSaveData(StructurePieceSerializationContext structurePieceSerializationContext, CompoundTag compoundTag) {
        ...
        RegistryOps<Tag> dynamicOps = structurePieceSerializationContext.registryAccess().createSerializationContext(NbtOps.INSTANCE);
        compoundTag.store("pool_element", StructurePoolElement.CODEC, dynamicOps, this.element);
        ...
        if (this.liquidSettings != JigsawStructure.DEFAULT_LIQUID_SETTINGS) {
            compoundTag.store("liquid_settings", LiquidSettings.CODEC, dynamicOps, this.liquidSettings);
        }
    }
