...
public final class RandomState {
            ...
            public DensityFunction.NoiseHolder visitNoise(DensityFunction.NoiseHolder noiseHolder) {
                ...
                if (bl) {
                    ...
                    if (holder.is(Noises.SHIFT)) {
                        NormalNoise normalNoise = NormalNoise.create(RandomState.this.random.fromHashOf(Noises.SHIFT.identifier()), new NormalNoise.NoiseParameters(0, 0.0, new double[0]));
                        return new DensityFunction.NoiseHolder(holder, normalNoise);
                    }
                }
                NormalNoise normalNoise = RandomState.this.getOrCreateNoise((ResourceKey)holder.unwrapKey().orElseThrow());
                return new DensityFunction.NoiseHolder(holder, normalNoise);
            }
    ...
    public PositionalRandomFactory getOrCreateRandomFactory(Identifier identifier) {
        return this.positionalRandoms.computeIfAbsent(identifier, identifier2 -> this.random.fromHashOf(identifier).forkPositional());
    }
