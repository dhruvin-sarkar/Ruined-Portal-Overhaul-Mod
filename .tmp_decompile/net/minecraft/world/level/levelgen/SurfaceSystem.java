...
public class SurfaceSystem {
        ...
        BlockColumn blockColumn = new BlockColumn(){
            ...
            public void setBlock(int i, BlockState blockState) {
                ...
                if (levelHeightAccessor.isInsideBuildHeight(i)) {
                    chunkAccess.setBlockState(mutableBlockPos.setY(i), blockState);
    ...
    private void erodedBadlandsExtension(BlockColumn blockColumn, int i, int j, int k, LevelHeightAccessor levelHeightAccessor) {
        ...
        for (n = m; n >= levelHeightAccessor.getMinY() && !(blockState = blockColumn.getBlock(n)).is(this.defaultBlock.getBlock()); --n) {
            if (!blockState.is(Blocks.WATER)) continue;
            return;
        }
        for (n = m; n >= levelHeightAccessor.getMinY() && blockColumn.getBlock(n).isAir(); --n) {
    ...
    private void frozenOceanExtension(int i, Biome biome, BlockColumn blockColumn, BlockPos.MutableBlockPos mutableBlockPos, int j, int k, int l) {
        ...
        for (int s = Math.max(l, (int)o + 1); s >= i; --s) {
            if (!(blockColumn.getBlock(s).isAir() && s < (int)o && randomSource.nextDouble() > 0.01) && (!blockColumn.getBlock(s).is(Blocks.WATER) || s <= (int)n || s >= this.seaLevel || n == 0.0 || !(randomSource.nextDouble() > 0.15))) continue;
            if (r <= p && s > q) {
