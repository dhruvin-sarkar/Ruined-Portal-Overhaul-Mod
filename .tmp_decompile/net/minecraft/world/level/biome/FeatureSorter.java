...
public class FeatureSorter {
    public static <T> List<StepFeatureData> buildFeaturesPerStep(List<T> list, Function<T, List<HolderSet<PlacedFeature>>> function, boolean bl) {
        ...
        for (T object2 : list) {
            ...
            for (j = 0; j < list3.size(); ++j) {
                for (Holder holder : (HolderSet)list3.get(j)) {
                    PlacedFeature placedFeature = (PlacedFeature)((Object)holder.value());
                    list2.add(new FeatureData(object2IntMap.computeIfAbsent((Object)placedFeature, object -> mutableInt.getAndIncrement()), j, placedFeature));
                }
            }
            for (j = 0; j < list2.size(); ++j) {
                Set set = map.computeIfAbsent((FeatureData)((Object)list2.get(j)), arg -> new TreeSet(comparator));
                if (j >= list2.size() - 1) continue;
                set.add((FeatureData)((Object)list2.get(j + 1)));
            }
        }
        ...
        for (FeatureData lv : map.keySet()) {
            ...
            if (set2.contains((Object)lv) || !Graph.depthFirstSearch(map, set2, set3, list2::add, lv)) continue;
            if (bl) {
                int k;
