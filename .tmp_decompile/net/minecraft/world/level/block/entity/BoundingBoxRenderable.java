...
public interface BoundingBoxRenderable {
    ...
    public record RenderableBox(BlockPos localPos, Vec3i size) {
        public static RenderableBox fromCorners(int i, int j, int k, int l, int m, int n) {
            int o = Math.min(i, l);
            int p = Math.min(j, m);
            int q = Math.min(k, n);
            return new RenderableBox(new BlockPos(o, p, q), new Vec3i(Math.max(i, l) - o, Math.max(j, m) - p, Math.max(k, n) - q));
        }
    }
}
