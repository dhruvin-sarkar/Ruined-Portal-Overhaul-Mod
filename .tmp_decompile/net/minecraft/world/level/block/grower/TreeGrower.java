...
public final class TreeGrower {
    ...
    private @Nullable ResourceKey<ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource randomSource, boolean bl) {
    ...
    private @Nullable ResourceKey<ConfiguredFeature<?, ?>> getConfiguredMegaFeature(RandomSource randomSource) {
    ...
    public boolean growTree(ServerLevel serverLevel, ChunkGenerator chunkGenerator, BlockPos blockPos, BlockState blockState, RandomSource randomSource) {
        ...
        if (resourceKey != null && (holder = (Holder)serverLevel.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(resourceKey).orElse(null)) != null) {
            for (int i = 0; i >= -1; --i) {
                for (int j = 0; j >= -1; --j) {
                    if (!TreeGrower.isTwoByTwoSapling(blockState, serverLevel, blockPos, i, j)) continue;
                    ConfiguredFeature configuredFeature = (ConfiguredFeature)((Object)holder.value());
        ...
        if ((resourceKey2 = this.getConfiguredFeature(randomSource, this.hasFlowers(serverLevel, blockPos))) == null) {
            return false;
        }
        Holder holder2 = serverLevel.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(resourceKey2).orElse(null);
        if (holder2 == null) {
            return false;
        }
        ConfiguredFeature configuredFeature2 = (ConfiguredFeature)((Object)holder2.value());
