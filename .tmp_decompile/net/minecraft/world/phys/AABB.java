...
public class AABB {
    ...
    public static Optional<Vec3> clip(double d, double e, double f, double g, double h, double i, Vec3 vec3, Vec3 vec32) {
        ...
        Direction direction = AABB.getDirection(d, e, f, g, h, i, vec3, ds, null, j, k, l);
        if (direction == null) {
    ...
    private static @Nullable Direction getDirection(AABB aABB, Vec3 vec3, double[] ds, @Nullable Direction direction, double d, double e, double f) {
        return AABB.getDirection(aABB.minX, aABB.minY, aABB.minZ, aABB.maxX, aABB.maxY, aABB.maxZ, vec3, ds, direction, d, e, f);
    }

    private static @Nullable Direction getDirection(double d, double e, double f, double g, double h, double i, Vec3 vec3, double[] ds, @Nullable Direction direction, double j, double k, double l) {
        if (j > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, j, k, l, d, e, h, f, i, Direction.WEST, vec3.x, vec3.y, vec3.z);
        } else if (j < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, j, k, l, g, e, h, f, i, Direction.EAST, vec3.x, vec3.y, vec3.z);
        }
        if (k > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, k, l, j, e, f, i, d, g, Direction.DOWN, vec3.y, vec3.z, vec3.x);
        } else if (k < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, k, l, j, h, f, i, d, g, Direction.UP, vec3.y, vec3.z, vec3.x);
        }
        if (l > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, l, j, k, f, d, g, e, h, Direction.NORTH, vec3.z, vec3.x, vec3.y);
        } else if (l < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, l, j, k, i, d, g, e, h, Direction.SOUTH, vec3.z, vec3.x, vec3.y);
        }
        return direction;
    }

    private static @Nullable Direction clipPoint(double[] ds, @Nullable Direction direction, double d, double e, double f, double g, double h, double i, double j, double k, Direction direction2, double l, double m, double n) {
        double o = (g - l) / d;
        double p = m + o * e;
        double q = n + o * f;
    ...
    public double distanceToSqr(AABB aABB) {
        ...
        double e = Math.max(Math.max(this.minY - aABB.maxY, aABB.minY - this.maxY), 0.0);
        double f = Math.max(Math.max(this.minZ - aABB.maxZ, aABB.minZ - this.maxZ), 0.0);
        return Mth.lengthSquared(d, e, f);
    }
