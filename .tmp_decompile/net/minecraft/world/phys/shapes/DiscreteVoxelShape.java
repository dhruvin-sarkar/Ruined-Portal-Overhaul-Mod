...
public abstract class DiscreteVoxelShape {
    ...
    public DiscreteVoxelShape rotate(OctahedralGroup octahedralGroup) {
        ...
        int i = DiscreteVoxelShape.fixupCoordinate(vector3i, 0);
        int j = DiscreteVoxelShape.fixupCoordinate(vector3i, 1);
        int k = DiscreteVoxelShape.fixupCoordinate(vector3i, 2);
        BitSetDiscreteVoxelShape discreteVoxelShape = new BitSetDiscreteVoxelShape(vector3i.x, vector3i.y, vector3i.z);
        for (int l = 0; l < this.xSize; ++l) {
            for (int m = 0; m < this.ySize; ++m) {
                for (int n = 0; n < this.zSize; ++n) {
                    if (!this.isFull(l, m, n)) continue;
                    Vector3i vector3i2 = octahedralGroup.rotate(vector3i.set(l, m, n));
                    int o = i + vector3i2.x;
                    int p = j + vector3i2.y;
                    int q = k + vector3i2.z;
                    ((DiscreteVoxelShape)discreteVoxelShape).fill(o, p, q);
                }
            }
        }
    ...
    private static int fixupCoordinate(Vector3i vector3i, int i) {
