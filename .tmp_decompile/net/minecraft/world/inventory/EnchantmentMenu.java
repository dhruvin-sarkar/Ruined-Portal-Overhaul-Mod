...
extends AbstractContainerMenu {
            ...
            if (itemStack.isEmpty() || !itemStack.isEnchantable()) {
                ...
                this.access.execute((level, blockPos) -> {
                    ...
                    for (j = 0; j < 3; ++j) {
                        ...
                        if (this.costs[j] <= 0 || (list = this.getEnchantmentList(level.registryAccess(), itemStack, j, this.costs[j])).isEmpty()) continue;
                        EnchantmentInstance enchantmentInstance = list.get(this.random.nextInt(list.size()));
                        this.enchantClue[j] = idMap.getId(enchantmentInstance.enchantment());
    ...
    public boolean clickMenuButton(Player player, int i) {
        ...
        if (this.costs[i] > 0 && !itemStack.isEmpty() && (player.experienceLevel >= j && player.experienceLevel >= this.costs[i] || player.hasInfiniteMaterials())) {
            this.access.execute((level, blockPos) -> {
                ItemStack itemStack3 = itemStack;
                List<EnchantmentInstance> list = this.getEnchantmentList(level.registryAccess(), itemStack3, i, this.costs[i]);
                if (!list.isEmpty()) {
