...
extends AbstractContainerMenu {
    ...
    void setupResultSlot(int i) {
        ...
        if (!this.recipesForInput.isEmpty() && this.isValidRecipeIndex(i)) {
            SelectableRecipe.SingleInputEntry<StonecutterRecipe> singleInputEntry = this.recipesForInput.entries().get(i);
            optional = singleInputEntry.recipe().recipe();
        } else {
            optional = Optional.empty();
        }
        optional.ifPresentOrElse(recipeHolder -> {
            this.resultContainer.setRecipeUsed((RecipeHolder<?>)((Object)recipeHolder));
    ...
    public ItemStack quickMoveStack(Player player, int i) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (i == 0 ? !this.moveItemStackTo(itemStack2, 2, 38, false) : (this.level.recipeAccess().stonecutterRecipes().acceptsInput(itemStack2) ? !this.moveItemStackTo(itemStack2, 0, 1, false) : (i >= 2 && i < 29 ? !this.moveItemStackTo(itemStack2, 29, 38, false) : i >= 29 && i < 38 && !this.moveItemStackTo(itemStack2, 2, 29, false)))) {
                return ItemStack.EMPTY;
            }
            if (itemStack2.isEmpty()) {
