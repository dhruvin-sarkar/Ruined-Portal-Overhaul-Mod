...
public class AcquirePoi {
    ...
    public static BehaviorControl<PathfinderMob> create(Predicate<Holder<PoiType>> predicate, MemoryModuleType<GlobalPos> memoryModuleType, MemoryModuleType<GlobalPos> memoryModuleType2, boolean bl, Optional<Byte> optional, BiPredicate<ServerLevel, BlockPos> biPredicate) {
        ...
        OneShot<PathfinderMob> oneShot = BehaviorBuilder.create(arg_0 -> AcquirePoi.method_46879(memoryModuleType2, bl, mutableLong, (Long2ObjectMap)long2ObjectMap, predicate, biPredicate, optional, arg_0));
    ...
    public static @Nullable Path findPathToPois(Mob mob, Set<Pair<Holder<PoiType>, BlockPos>> set) {
        if (set.isEmpty()) {
            return null;
        }
        ...
        for (Pair<Holder<PoiType>, BlockPos> pair : set) {
            i = Math.max(i, ((PoiType)((Object)((Holder)pair.getFirst()).value())).validRange());
            set2.add((BlockPos)pair.getSecond());
        }
        return mob.getNavigation().createPath(set2, i);
    }

    private static /* synthetic */ App method_46879(MemoryModuleType memoryModuleType, boolean bl, MutableLong mutableLong, Long2ObjectMap long2ObjectMap, Predicate predicate, BiPredicate biPredicate, Optional optional, BehaviorBuilder.Instance instance) {
        return instance.group(instance.absent(memoryModuleType)).apply((Applicative)instance, memoryAccessor -> (serverLevel, pathfinderMob, l) -> {
            ...
            if (serverLevel.getGameTime() < mutableLong.longValue()) {
                return false;
            }
            ...
            if (path != null && path.canReach()) {
                ...
                for (Pair<Holder<PoiType>, BlockPos> pair2 : set) {
                    long2ObjectMap.computeIfAbsent(((BlockPos)pair2.getSecond()).asLong(), m -> new JitteredLinearRetry(serverLevel.random, l));
                }
            }
            return true;
        });
