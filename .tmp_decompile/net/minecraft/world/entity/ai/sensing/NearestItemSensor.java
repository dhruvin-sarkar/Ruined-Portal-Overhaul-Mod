...
extends Sensor<Mob> {
    ...
    protected void doTick(ServerLevel serverLevel, Mob mob) {
        ...
        Optional<ItemEntity> optional = list.stream().filter(itemEntity -> mob.wantsToPickUp(serverLevel, itemEntity.getItem())).filter(itemEntity -> itemEntity.closerThan(mob, 32.0)).filter(mob::hasLineOfSight).findFirst();
