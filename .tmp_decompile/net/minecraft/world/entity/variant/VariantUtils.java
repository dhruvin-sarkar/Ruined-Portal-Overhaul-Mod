...
public class VariantUtils {
    ...
    public static <T> Holder<T> getDefaultOrAny(RegistryAccess registryAccess, ResourceKey<T> resourceKey) {
        HolderLookup.RegistryLookup registry = registryAccess.lookupOrThrow(resourceKey.registryKey());
        return (Holder)registry.get(resourceKey).or(((Registry)registry)::getAny).orElseThrow();
    }

    public static <T> Holder<T> getAny(RegistryAccess registryAccess, ResourceKey<? extends Registry<T>> resourceKey) {
        return (Holder)registryAccess.lookupOrThrow(resourceKey).getAny().orElseThrow();
    }

    public static <T> void writeVariant(ValueOutput valueOutput, Holder<T> holder) {
        holder.unwrapKey().ifPresent(resourceKey -> valueOutput.store(TAG_VARIANT, Identifier.CODEC, resourceKey.identifier()));
    }

    public static <T> Optional<Holder<T>> readVariant(ValueInput valueInput, ResourceKey<? extends Registry<T>> resourceKey) {
        return valueInput.read(TAG_VARIANT, Identifier.CODEC).map(identifier -> ResourceKey.create(resourceKey, identifier)).flatMap(valueInput.lookup()::get);
    }
