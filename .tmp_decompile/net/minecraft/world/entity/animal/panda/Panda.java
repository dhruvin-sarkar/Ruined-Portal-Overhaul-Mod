...
extends Animal {
    ...
    private void addEatingParticles() {
        if (this.getEatCounter() % 5 == 0) {
            ...
            for (int i = 0; i < 6; ++i) {
                Vec3 vec3 = new Vec3(((double)this.random.nextFloat() - 0.5) * 0.1, (double)this.random.nextFloat() * 0.1 + 0.1, ((double)this.random.nextFloat() - 0.5) * 0.1);
    ...
    extends AvoidEntityGoal<T> {
        ...
        public PandaAvoidGoal(Panda panda, Class<T> class_, float f, double d, double e) {
            super(panda, class_, f, d, e, EntitySelector.NO_SPECTATORS);
            this.panda = panda;
        }
