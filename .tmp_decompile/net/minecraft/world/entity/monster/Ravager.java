...
extends Raider {
    ...
    private void roar() {
        ...
        if (this.isAlive() && (level = this.level()) instanceof ServerLevel) {
            ServerLevel serverLevel = (ServerLevel)level;
            Predicate<Entity> predicate = serverLevel.getGameRules().get(GameRules.MOB_GRIEFING) != false ? ROAR_TARGET_WITH_GRIEFING : ROAR_TARGET_WITHOUT_GRIEFING;
            List<Entity> list = this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0), predicate);
    ...
    protected AABB getAttackBoundingBox(double d) {
        AABB aABB = super.getAttackBoundingBox(d);
