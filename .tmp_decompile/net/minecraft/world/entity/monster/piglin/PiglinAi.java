...
public class PiglinAi {
    ...
    private static void initFightActivity(Piglin piglin, Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, (ImmutableList<BehaviorControl<Piglin>>)ImmutableList.of(StopAttackingIfTargetInvalid.create((serverLevel, livingEntity) -> !PiglinAi.isNearestValidAttackTarget(serverLevel, piglin, livingEntity)), BehaviorBuilder.triggerIf(PiglinAi::hasCrossbow, BackUpIfTooClose.create(5, 0.75f)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0f), (Object)new SpearApproach(1.0, 10.0f), (Object)new SpearAttack(1.0, 1.0, 10.0f, 2.0f), (Object)new SpearRetreat(1.0), MeleeAttack.create(20), new CrossbowAttack(), RememberIfHoglinWasKilled.create(), EraseMemoryIf.create(PiglinAi::isNearZombified, MemoryModuleType.ATTACK_TARGET)), MemoryModuleType.ATTACK_TARGET);
    }
    ...
    protected static void stopHoldingOffHandItem(ServerLevel serverLevel, Piglin piglin, boolean bl) {
        ...
        if (piglin.isAdult()) {
            ...
            boolean bl2;
            boolean bl5 = bl2 = !piglin.equipItemIfPossible(serverLevel, itemStack).isEmpty();
            if (!bl2) {
    ...
    private static List<ItemStack> getBarterResponseItems(Piglin piglin) {
        ...
        ObjectArrayList<ItemStack> list = lootTable.getRandomItems(new LootParams.Builder((ServerLevel)piglin.level()).withParameter(LootContextParams.THIS_ENTITY, piglin).create(LootContextParamSets.PIGLIN_BARTER));
        return list;
    }
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel serverLevel, Piglin piglin) {
        ...
        if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(serverLevel, piglin, optional.get())) {
            return optional;
        }
    ...
    public static void angerNearbyPiglins(ServerLevel serverLevel, Player player, boolean bl) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(piglin -> !bl || BehaviorUtils.canSee(piglin, player)).forEach(piglin -> {
            if (serverLevel.getGameRules().get(GameRules.UNIVERSAL_ANGER).booleanValue()) {
    ...
    protected static void maybeRetaliate(ServerLevel serverLevel, AbstractPiglin abstractPiglin, LivingEntity livingEntity) {
        ...
        if (livingEntity.getType() == EntityType.PLAYER && serverLevel.getGameRules().get(GameRules.UNIVERSAL_ANGER).booleanValue()) {
