...
implements Enemy {
    ...
    extends MoveControl {
        ...
        private boolean canReach(Vec3 vec3) {
            ...
            return BlockGetter.forEachBlockIntersectedBetween(vec32, vec33, aABB2, (blockPos, i) -> {
        ...
        private boolean blockTraversalPossible(BlockGetter blockGetter, @Nullable Vec3 vec3, @Nullable Vec3 vec32, BlockPos blockPos, boolean bl, boolean bl2) {
            ...
            boolean bl5 = bl3 = vec3 != null && vec32 != null;
            boolean bl6 = bl3 ? !this.ghast.collidedWithShapeMovingFrom(vec3, vec32, blockState.getCollisionShape(blockGetter, blockPos).move(new Vec3(blockPos)).toAabbs()) : (bl4 = blockState.getCollisionShape(blockGetter, blockPos).isEmpty());
            if (!this.careful) {
                return bl4;
            }
    ...
    extends Goal {
        ...
        public static Vec3 getSuitableFlyToPosition(Mob mob, int i) {
            ...
            for (int j = 0; j < 64; ++j) {
                ...
                if (vec32 == null || !RandomFloatAroundGoal.isGoodTarget(level, vec32, i)) continue;
                return vec32;
            }
            if (vec32 == null) {
                vec32 = RandomFloatAroundGoal.chooseRandomPosition(vec3, randomSource);
            }
            if ((k = level.getHeight(Heightmap.Types.MOTION_BLOCKING, (blockPos = BlockPos.containing(vec32)).getX(), blockPos.getZ())) < blockPos.getY() && k > level.getMinY()) {
        ...
        private static @Nullable Vec3 chooseRandomPositionWithRestriction(Mob mob, Vec3 vec3, RandomSource randomSource) {
