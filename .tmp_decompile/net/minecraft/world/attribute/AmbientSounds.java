...
public record AmbientSounds(Optional<Holder<SoundEvent>> loop, Optional<AmbientMoodSettings> mood, List<AmbientAdditionsSettings> additions) {
    ...
    public static final Codec<AmbientSounds> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)SoundEvent.CODEC.optionalFieldOf("loop").forGetter(AmbientSounds::loop), (App)AmbientMoodSettings.CODEC.optionalFieldOf("mood").forGetter(AmbientSounds::mood), (App)ExtraCodecs.compactListCodec(AmbientAdditionsSettings.CODEC).optionalFieldOf("additions", (Object)List.of()).forGetter(AmbientSounds::additions)).apply((Applicative)instance, AmbientSounds::new));
}
