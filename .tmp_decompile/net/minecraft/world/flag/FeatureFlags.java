...
public class FeatureFlags {
    ...
    public static String printMissingFlags(FeatureFlagRegistry featureFlagRegistry, FeatureFlagSet featureFlagSet, FeatureFlagSet featureFlagSet2) {
        Set<Identifier> set = featureFlagRegistry.toNames(featureFlagSet2);
        Set<Identifier> set2 = featureFlagRegistry.toNames(featureFlagSet);
        return set.stream().filter(identifier -> !set2.contains(identifier)).map(Identifier::toString).collect(Collectors.joining(", "));
    }
