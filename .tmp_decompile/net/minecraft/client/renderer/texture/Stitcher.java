...
public class Stitcher<T extends Entry> {
    ...
    public Stitcher(int i, int j, int k, int l) {
        this.mipLevel = k;
        this.maxWidth = i;
        this.maxHeight = j;
        this.padding = 1 << k << Mth.clamp(l - 1, 0, 4);
    }
    ...
    public void stitch() {
        ...
        for (Holder holder2 : list) {
            ...
            throw new StitcherException((Entry)holder2.entry, (Collection)list.stream().map(holder -> holder.entry).collect(ImmutableList.toImmutableList()));
        }
    }

    public void gatherSprites(SpriteLoader<T> spriteLoader) {
        for (Region<T> region : this.storage) {
            region.walk(spriteLoader, this.padding);
        }
    }
    ...
    public static class Region<T extends Entry> {
        ...
        public void walk(SpriteLoader<T> spriteLoader, int i) {
            if (this.holder != null) {
                spriteLoader.load(this.holder.entry, this.getX(), this.getY(), i);
            } else if (this.subSlots != null) {
                for (Region region : this.subSlots) {
                    region.walk(spriteLoader, i);
                }
            }
        }
