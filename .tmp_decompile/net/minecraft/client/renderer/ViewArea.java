...
public class ViewArea {
        ...
        for (int j = 0; j < this.sectionGridSizeX; ++j) {
            for (int k = 0; k < this.sectionGridSizeY; ++k) {
                for (int l = 0; l < this.sectionGridSizeZ; ++l) {
                    ...
                    this.sections[m] = new SectionRenderDispatcher.RenderSection(sectionRenderDispatcher2, m, SectionPos.asLong(j, k + this.level.getMinSectionY(), l));
                }
            }
        }
    }
        ...
        for (int i = 0; i < this.sectionGridSizeX; ++i) {
            ...
            for (int l = 0; l < this.sectionGridSizeZ; ++l) {
                ...
                for (int o = 0; o < this.sectionGridSizeY; ++o) {
                    ...
                    if (q == SectionPos.asLong(k, p, n)) continue;
    ...
    protected @Nullable SectionRenderDispatcher.RenderSection getRenderSectionAt(BlockPos blockPos) {
        return this.getRenderSection(SectionPos.asLong(blockPos));
    }

    protected @Nullable SectionRenderDispatcher.RenderSection getRenderSection(long l) {
    ...
    private @Nullable SectionRenderDispatcher.RenderSection getRenderSection(int i, int j, int k) {
    ...
    private boolean containsSection(int i, int j, int k) {
        ...
        if (i < this.cameraSectionPos.x() - this.viewDistance || i > this.cameraSectionPos.x() + this.viewDistance) {
            return false;
        }
        return k >= this.cameraSectionPos.z() - this.viewDistance && k <= this.cameraSectionPos.z() + this.viewDistance;
    }
}
