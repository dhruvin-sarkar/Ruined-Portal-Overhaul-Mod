...
public class ItemModelGenerators {
    ...
    public static Identifier prefixForSlotTrim(String string) {
        return Identifier.withDefaultNamespace("trims/items/" + string + "_trim");
    }

    public ItemModelGenerators(ItemModelOutput itemModelOutput, BiConsumer<Identifier, ModelInstance> biConsumer) {
    ...
    private Identifier createFlatItemModel(Item item, ModelTemplate modelTemplate) {
    ...
    private Identifier createFlatItemModel(Item item, String string, ModelTemplate modelTemplate) {
    ...
    private Identifier createFlatItemModel(Item item, Item item2, ModelTemplate modelTemplate) {
    ...
    private void generateItemWithTintedOverlay(Item item, String string, ItemTintSource itemTintSource) {
        Identifier identifier = this.generateLayeredItem(item, TextureMapping.getItemTexture(item), TextureMapping.getItemTexture(item, string));
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier, BLANK_LAYER, itemTintSource));
    }

    private void generateItemWithTintedBaseLayer(Item item, int i) {
        Identifier identifier = TextureMapping.getItemTexture(item);
        Identifier identifier2 = TextureMapping.getItemTexture(item, "_overlay");
        Identifier identifier3 = ModelLocationUtils.getModelLocation(item);
        ModelTemplates.TWO_LAYERED_ITEM.create(identifier3, TextureMapping.layered(identifier, identifier2), this.modelOutput);
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier3, new Dye(i)));
    }
    ...
    private List<RangeSelectItemModel.Entry> createCompassModels(Item item) {
        ...
        for (int i = 1; i < 32; ++i) {
            ...
            list.add(ItemModelUtils.override(unbaked2, (float)i - 0.5f));
        }
    ...
    private void generateStandardCompassItem(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.hasComponent(DataComponents.LODESTONE_TRACKER), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.LODESTONE), 32.0f, list), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.SPAWN), 32.0f, list)));
    }
    ...
    private void generateClockItem(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.inOverworld(ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.DAYTIME), 64.0f, list), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.RANDOM), 64.0f, list)));
    }

    private Identifier generateLayeredItem(Item item, Identifier identifier, Identifier identifier2) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(item, TextureMapping.layered(identifier, identifier2), this.modelOutput);
    }

    private Identifier generateLayeredItem(Identifier identifier, Identifier identifier2, Identifier identifier3) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(identifier, TextureMapping.layered(identifier2, identifier3), this.modelOutput);
    }

    private void generateLayeredItem(Identifier identifier, Identifier identifier2, Identifier identifier3, Identifier identifier4) {
        ModelTemplates.THREE_LAYERED_ITEM.create(identifier, TextureMapping.layered(identifier2, identifier3, identifier4), this.modelOutput);
    }

    private void generateTrimmableItem(Item item, ResourceKey<EquipmentAsset> resourceKey, Identifier identifier, boolean bl) {
        ...
        ArrayList list = new ArrayList(TRIM_MATERIAL_MODELS.size());
        for (TrimMaterialData trimMaterialData : TRIM_MATERIAL_MODELS) {
            ...
            if (bl) {
                this.generateLayeredItem(identifier5, identifier3, identifier4, identifier6);
                unbaked = ItemModelUtils.tintedModel(identifier5, new Dye(-6265536));
            } else {
                this.generateLayeredItem(identifier5, identifier3, identifier6);
                unbaked = ItemModelUtils.plainModel(identifier5);
            }
            list.add(ItemModelUtils.when(trimMaterialData.materialKey, unbaked));
        }
        if (bl) {
            ModelTemplates.TWO_LAYERED_ITEM.create(identifier2, TextureMapping.layered(identifier3, identifier4), this.modelOutput);
            unbaked2 = ItemModelUtils.tintedModel(identifier2, new Dye(-6265536));
        } else {
            ModelTemplates.FLAT_ITEM.create(identifier2, TextureMapping.layer0(identifier3), this.modelOutput);
    ...
    private void generateBundleModels(Item item) {
        ...
        Identifier identifier = this.generateBundleCoverModel(item, ModelTemplates.BUNDLE_OPEN_BACK_INVENTORY, "_open_back");
        Identifier identifier2 = this.generateBundleCoverModel(item, ModelTemplates.BUNDLE_OPEN_FRONT_INVENTORY, "_open_front");
        ItemModel.Unbaked unbaked2 = ItemModelUtils.composite(ItemModelUtils.plainModel(identifier), new BundleSelectedItemSpecialRenderer.Unbaked(), ItemModelUtils.plainModel(identifier2));
    ...
    private Identifier generateBundleCoverModel(Item item, ModelTemplate modelTemplate, String string) {
        Identifier identifier = TextureMapping.getItemTexture(item, string);
        return modelTemplate.create(item, TextureMapping.layer0(identifier), this.modelOutput);
    }
    ...
    private static ItemModel.Unbaked createFlatModelDispatch(ItemModel.Unbaked unbaked, ItemModel.Unbaked unbaked2) {
        return ItemModelUtils.select(new DisplayContext(), unbaked2, ItemModelUtils.when(List.of((Object)ItemDisplayContext.GUI, (Object)ItemDisplayContext.GROUND, (Object)ItemDisplayContext.FIXED, (Object)ItemDisplayContext.ON_SHELF), unbaked));
    }
    ...
    private void generateSpear(Item item) {
        ItemModel.Unbaked unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(item, ModelTemplates.FLAT_ITEM));
        ItemModel.Unbaked unbaked2 = ItemModelUtils.plainModel(ModelTemplates.SPEAR_IN_HAND.create(item, TextureMapping.layer0(TextureMapping.getItemTexture(item, "_in_hand")), this.modelOutput));
        this.itemModelOutput.accept(item, ItemModelGenerators.createFlatModelDispatch(unbaked, unbaked2), new ClientItem.Properties(true, false, 1.95f));
    }

    private void addPotionTint(Item item, Identifier identifier) {
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier, new Potion()));
    }

    private void generatePotion(Item item) {
        Identifier identifier = this.generateLayeredItem(item, ModelLocationUtils.decorateItemModelLocation("potion_overlay"), ModelLocationUtils.getModelLocation(item));
        this.addPotionTint(item, identifier);
    }

    private void generateTippedArrow(Item item) {
        Identifier identifier = this.generateLayeredItem(item, ModelLocationUtils.getModelLocation(item, "_head"), ModelLocationUtils.getModelLocation(item, "_base"));
        this.addPotionTint(item, identifier);
    }

    private void generateDyedItem(Item item, int i) {
        Identifier identifier = this.createFlatItemModel(item, ModelTemplates.FLAT_ITEM);
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier, new Dye(i)));
    }

    private void generateTwoLayerDyedItem(Item item) {
        Identifier identifier = TextureMapping.getItemTexture(item);
        Identifier identifier2 = TextureMapping.getItemTexture(item, "_overlay");
        Identifier identifier3 = ModelTemplates.FLAT_ITEM.create(item, TextureMapping.layer0(identifier), this.modelOutput);
        Identifier identifier4 = ModelLocationUtils.getModelLocation(item, "_dyed");
        ModelTemplates.TWO_LAYERED_ITEM.create(identifier4, TextureMapping.layered(identifier, identifier2), this.modelOutput);
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.hasComponent(DataComponents.DYED_COLOR), ItemModelUtils.tintedModel(identifier4, BLANK_LAYER, new Dye(0)), ItemModelUtils.plainModel(identifier3)));
    }
