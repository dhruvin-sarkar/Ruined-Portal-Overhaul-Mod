...
implements DataProvider {
    ...
    implements ItemModelOutput {
        ...
        public void finalizeAndValidate() {
            ...
            List list = BuiltInRegistries.ITEM.listElements().filter(reference -> !this.itemInfos.containsKey(reference.value())).map(reference -> reference.key().identifier()).toList();
            if (!list.isEmpty()) {
        ...
        public CompletableFuture<?> save(CachedOutput cachedOutput, PackOutput.PathProvider pathProvider) {
            return DataProvider.saveAll(cachedOutput, ClientItem.CODEC, item -> pathProvider.json(item.builtInRegistryHolder().key().identifier()), this.itemInfos);
        }
    }
    ...
    implements Consumer<BlockModelDefinitionGenerator> {
        ...
        public void validate() {
            ...
            List list = stream.filter(reference -> !this.generators.containsKey(reference.value())).map(reference -> reference.key().identifier()).toList();
            if (!list.isEmpty()) {
        ...
        public CompletableFuture<?> save(CachedOutput cachedOutput, PackOutput.PathProvider pathProvider) {
            ...
            Function<Block, Path> function = block -> pathProvider.json(block.builtInRegistryHolder().key().identifier());
    ...
    implements BiConsumer<Identifier, ModelInstance> {
        ...
        @Override
        public void accept(Identifier identifier, ModelInstance modelInstance) {
            Supplier supplier = this.models.put(identifier, modelInstance);
            if (supplier != null) {
        ...
        public /* synthetic */ void accept(Object object, Object object2) {
            this.accept((Identifier)object, (ModelInstance)object2);
        }
    }
}
