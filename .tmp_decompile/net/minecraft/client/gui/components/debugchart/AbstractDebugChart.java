...
public abstract class AbstractDebugChart {
    ...
    public void drawChart(GuiGraphics guiGraphics, int i, int j) {
        ...
        guiGraphics.fill(i, k - 60, i + j, k, -1873784752);
        long l = 0L;
        ...
        guiGraphics.hLine(i, i + j - 1, k - 60, -1);
        guiGraphics.hLine(i, i + j - 1, k - 1, -1);
        guiGraphics.vLine(i, k - 60, k, -1);
        guiGraphics.vLine(i + j - 1, k - 60, k, -1);
        if (p > 0) {
            ...
            guiGraphics.drawString(this.font, string, i + 2, k - 60 - this.font.lineHeight, -2039584);
            guiGraphics.drawCenteredString(this.font, string2, i + j / 2, k - 60 - this.font.lineHeight, -2039584);
            guiGraphics.drawString(this.font, string3, i + j - this.font.width(string3) - 2, k - 60 - this.font.lineHeight, -2039584);
        }
    ...
    protected void drawMainDimension(GuiGraphics guiGraphics, int i, int j, int k) {
        ...
        int m = this.getSampleHeight(l);
        int n = this.getSampleColor(l);
        guiGraphics.fill(j, i - m, j + 1, i, n);
    }
    ...
    protected void drawStringWithShade(GuiGraphics guiGraphics, String string, int i, int j) {
        guiGraphics.fill(i, j, i + this.font.width(string) + 1, j + this.font.lineHeight, -1873784752);
        guiGraphics.drawString(this.font, string, i + 1, j + 1, -2039584, false);
    }
    ...
    protected int getSampleColor(double d, double e, int i, double f, int j, double g, int k) {
        if ((d = Mth.clamp(d, e, g)) < f) {
            return ARGB.srgbLerp((float)((d - e) / (f - e)), i, j);
        }
        return ARGB.srgbLerp((float)((d - f) / (g - f)), j, k);
    }
}
