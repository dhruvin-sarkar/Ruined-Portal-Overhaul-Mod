...
public class AdvancementWidget {
    ...
    public AdvancementWidget(AdvancementTab advancementTab, Minecraft minecraft, AdvancementNode advancementNode, DisplayInfo displayInfo) {
        ...
        int k = 29 + i + j;
        this.description = Language.getInstance().getVisualOrder(this.findOptimalLines(ComponentUtils.mergeStyles(displayInfo.getDescription(), Style.EMPTY.withColor(displayInfo.getType().getChatColor())), k));
    ...
    private int getMaxProgressWidth() {
        ...
        int j = 8;
        MutableComponent component = Component.translatable("advancements.progress", i, i);
    ...
    private @Nullable AdvancementWidget getFirstVisibleParent(AdvancementNode advancementNode) {
    ...
    public void drawHover(GuiGraphics guiGraphics, int i, int j, float f, int k, int l) {
        ...
        int m = font.lineHeight * this.titleLines.size() + 9 + 8;
        int n = j + this.y + (26 - m) / 2;
        int o = n + m;
        int p = this.description.size() * font.lineHeight;
        int q = 6 + p;
        ...
        int s = Mth.floor(g * (float)this.width);
        if (g >= 1.0f) {
            s = this.width / 2;
        ...
        int t = this.width - s;
        int u = bl ? i + this.x - this.width + 26 + 6 : i + this.x;
        int v = m + q;
    ...
    private void drawMultilineText(GuiGraphics guiGraphics, List<FormattedCharSequence> list, int i, int j, int k) {
        ...
        for (int l = 0; l < list.size(); ++l) {
            guiGraphics.drawString(font, list.get(l), i, j + l * font.lineHeight, k);
        }
    }
