...
public class ChatSelectionLogFiller {
    ...
    public void fillNextPage(int i, Output output) {
        ...
        while (j < i && (loggedChatEvent = this.log.lookup(this.eventId)) != null) {
            ...
            int k = this.eventId--;
            if (!(loggedChatEvent instanceof LoggedChatMessage.Player) || (player = (LoggedChatMessage.Player)loggedChatEvent).message().equals((Object)this.lastMessage)) continue;
    ...
    private boolean acceptMessage(Output output, LoggedChatMessage.Player player) {
        ...
        if (this.canReport.test(player)) {
            ...
            if (this.previousLink != null && !this.previousLink.isDescendantOf(playerChatMessage.link())) {
                output.acceptDivider(Component.translatable("gui.chatSelection.join", player.profile().name()).withStyle(ChatFormatting.YELLOW));
            }
