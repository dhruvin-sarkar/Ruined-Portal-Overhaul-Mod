...
implements ClientTooltipComponent {
    ...
    private void renderEmptyBundleTooltip(Font font, int i, int j, int k, int l, GuiGraphics guiGraphics) {
        ClientBundleTooltip.drawEmptyBundleDescriptionText(i + this.getContentXOffset(k), j, font, guiGraphics);
        this.drawProgressbar(i + this.getContentXOffset(k), j + ClientBundleTooltip.getEmptyBundleDescriptionTextHeight(font) + 4, font, guiGraphics);
    }
    ...
    private void renderBundleWithItemsTooltip(Font font, int i, int j, int k, int l, GuiGraphics guiGraphics) {
        ...
        for (int p = 1; p <= this.gridSizeY(); ++p) {
            for (int q = 1; q <= 4; ++q) {
                int r = m - q * 24;
                int s = n - p * 24;
                if (ClientBundleTooltip.shouldRenderSurplusText(bl, q, p)) {
                    ClientBundleTooltip.renderCount(r, s, this.getAmountOfHiddenItems(list), font, guiGraphics);
                    continue;
                }
                if (!ClientBundleTooltip.shouldRenderItemSlot(list, o)) continue;
    ...
    private void drawSelectedItemTooltip(Font font, GuiGraphics guiGraphics, int i, int j, int k) {
        if (this.contents.hasSelectedItem()) {
            ...
            guiGraphics.renderTooltip(font, List.of((Object)clientTooltipComponent), m - l / 2, j - 15, DefaultTooltipPositioner.INSTANCE, itemStack.get(DataComponents.TOOLTIP_STYLE));
        }
    }
    ...
    private static void drawEmptyBundleDescriptionText(int i, int j, Font font, GuiGraphics guiGraphics) {
        guiGraphics.drawWordWrap(font, BUNDLE_EMPTY_DESCRIPTION, i, j, 96, -5592406);
    }
