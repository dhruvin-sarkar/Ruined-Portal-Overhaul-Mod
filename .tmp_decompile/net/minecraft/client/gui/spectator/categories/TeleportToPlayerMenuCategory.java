...
SpectatorMenuItem {
    ...
    public TeleportToPlayerMenuCategory(Collection<PlayerInfo> collection) {
        this.items = (List)collection.stream().filter(playerInfo -> playerInfo.getGameMode() != GameType.SPECTATOR).sorted(PROFILE_ORDER).map(PlayerMenuItem::new).collect(Collectors.toUnmodifiableList());
    }
    ...
    public void renderIcon(GuiGraphics guiGraphics, float f, float g) {
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TELEPORT_TO_PLAYER_SPRITE, 0, 0, 16, 16, ARGB.colorFromFloat(g, f, f, f));
    }
