...
extends EntityModel<HoglinRenderState> {
    ...
    public void setupAnim(HoglinRenderState hoglinRenderState) {
        ...
        this.rightEar.zRot = -0.6981317f - f * Mth.sin(g);
        this.leftEar.zRot = 0.6981317f + f * Mth.sin(g);
        ...
        this.head.xRot = Mth.lerp(h, 0.87266463f, -0.34906584f);
        if (hoglinRenderState.isBaby) {
            this.head.y += h * 2.5f;
        }
        float i = 1.2f;
        this.rightFrontLeg.xRot = Mth.cos(g) * 1.2f * f;
        this.rightHindLeg.xRot = this.leftFrontLeg.xRot = Mth.cos(g + (float)Math.PI) * 1.2f * f;
