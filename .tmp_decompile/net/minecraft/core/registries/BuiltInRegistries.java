...
public class BuiltInRegistries {
    ...
    private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> resourceKey, R writableRegistry, RegistryBootstrap<T> registryBootstrap) {
        Bootstrap.checkBootstrapCalled(() -> "registry " + String.valueOf(resourceKey.identifier()));
        Identifier identifier = resourceKey.identifier();
        LOADERS.put(identifier, () -> registryBootstrap.run(writableRegistry));
    ...
    private static <T extends Registry<?>> void validate(Registry<T> registry) {
        registry.forEach(registry2 -> {
            ...
            if (registry2 instanceof DefaultedRegistry) {
                Identifier identifier = ((DefaultedRegistry)registry2).getDefaultKey();
                Objects.requireNonNull(registry2.getValue(identifier), "Missing default of DefaultedMappedRegistry: " + String.valueOf(identifier));
            }
        });
    }
