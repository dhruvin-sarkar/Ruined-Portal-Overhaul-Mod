...
public enum FrontAndTop implements StringRepresentable
{
    ...
    static {
        ...
        BY_TOP_FRONT = Util.make(new FrontAndTop[NUM_DIRECTIONS * NUM_DIRECTIONS], frontAndTops -> {
            FrontAndTop[] frontAndTopArray = FrontAndTop.values();
            int n = frontAndTopArray.length;
            for (int i = 0; i < n; ++i) {
                FrontAndTop frontAndTop;
                frontAndTops[FrontAndTop.lookupKey((Direction)frontAndTop.front, (Direction)frontAndTop.top)] = frontAndTop = frontAndTopArray[i];
            }
        });
    }
}
