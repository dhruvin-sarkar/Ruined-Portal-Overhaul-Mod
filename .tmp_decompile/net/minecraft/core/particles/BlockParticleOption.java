...
implements ParticleOptions {
    ...
    public static MapCodec<BlockParticleOption> codec(ParticleType<BlockParticleOption> particleType) {
        return BLOCK_STATE_CODEC.xmap(blockState -> new BlockParticleOption(particleType, (BlockState)blockState), blockParticleOption -> blockParticleOption.state).fieldOf("block_state");
    }
    ...
    public static StreamCodec<? super RegistryFriendlyByteBuf, BlockParticleOption> streamCodec(ParticleType<BlockParticleOption> particleType) {
        return ByteBufCodecs.idMapper(Block.BLOCK_STATE_REGISTRY).map(blockState -> new BlockParticleOption(particleType, (BlockState)blockState), blockParticleOption -> blockParticleOption.state);
    }
