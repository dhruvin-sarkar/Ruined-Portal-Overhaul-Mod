...
public class SharedConstants {
    ...
    private static String prefixDebugFlagName(String string) {
        return DEBUG_FLAG_PREFIX + string;
    }

    private static boolean booleanProperty(String string) {
        String string2 = System.getProperty(string);
        return string2 != null && (string2.isEmpty() || Boolean.parseBoolean(string2));
    }

    private static boolean debugFlag(String string) {
        if (!DEBUG_ENABLED) {
            return false;
        }
        String string2 = SharedConstants.prefixDebugFlagName(string);
        if (DEBUG_PRINT_PROPERTIES) {
            System.out.println("Debug property available: " + string2 + ": bool");
        }
        return SharedConstants.booleanProperty(string2);
    }

    private static int debugIntValue(String string) {
        if (!DEBUG_ENABLED) {
            return 0;
        }
        String string2 = SharedConstants.prefixDebugFlagName(string);
