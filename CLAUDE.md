...
Last reconciled: 2026-04-18. Current build status: `./gradlew build` succeeds with Java 21 when `JAVA_HOME` points at `C:\Users\dhruv\.codex\jdks\temurin-21`.
...
- Accessories API: required dependency `io.wispforest:accessories-fabric:1.4.3-beta+1.21.10`, used with Minecraft `1.21.11`
- Mod ID: `ruined_portal_overhaul`
- Package root: `com.ruinedportaloverhaul`

Accessories Maven repositories in `build.gradle`:

- `https://maven.wispforest.io/releases`
- `https://maven.su5ed.dev/releases`
- `https://maven.fabricmc.net`

`fabric.mod.json` depends on `"accessories": "*"`.
...
src/main/resources/data/accessories/tags/item/necklace.json
src/main/resources/data/accessories/tags/slot/necklace.json
src/main/resources/data/minecraft/worldgen/structure/ruined_portal*.json
src/main/resources/data/ruined_portal_overhaul/accessories/slot/necklace.json
...
1. Accessories API integration defines a custom necklace slot.
...
4. The Ghast Tear Necklace uses Accessories and the new nether-star economy.
...
Accessories is a required dependency, not optional. Keep these conventions:

- `GhastTearNecklaceItem` extends `io.wispforest.accessories.api.core.AccessoryItem`.
- Custom slots are data-driven under `data/<modid>/accessories/slot/`.
- The mod slot is `necklace`, defined at `data/ruined_portal_overhaul/accessories/slot/necklace.json` with `amount: 1`, `operation: set`, and chest-area icon metadata.
- Official Accessories item binding is `data/accessories/tags/item/necklace.json`.
- `data/accessories/tags/slot/necklace.json` is also present because the original expansion prompt requested that path; keep both in sync.
- Server-side equipped item lookups use `AccessoriesCapability.getOptionally(player)`.
...
The Ghast Tear Necklace is the first Accessories item:

- `ModItems`: registers `ghast_tear_necklace`, stack size 1, fire resistant.
- `GhastTearNecklaceItem`: extends `AccessoryItem`, triggers the equip advancement from `onEquip`, and applies Speed II plus Jump Boost II while equipped.
- `ModDataComponents`: registers `last_necklace_fireball_tick` as a persistent/networked long component on the equipped stack.
- `NetherFireballPayload`: empty C2S payload for the keybind ability.
- `NetherFireballHandler`: server-side capability lookup, cooldown check, fireball spawn, sound, stack component update, and advancement trigger.
...
Passive effects refresh every 40 ticks with 80-tick duration. The fireball ability has a 2400-tick cooldown, spawns a vanilla `SmallFireball` owned by the player, and uses `SoundEvents.GHAST_SHOOT`.
...
- Required dependency: Accessories API (`accessories`)
...
- Accessories code should use `AccessoryItem`, `SlotReference`, and `AccessoriesCapability`; do not port Trinkets/Curios API assumptions into this project.
- Accessory slot JSON belongs under `data/ruined_portal_overhaul/accessories/slot/`; item-slot binding belongs under `data/accessories/tags/item/`.
...
| Accessories API dependency, necklace slot, and slot binding tags | COMPLETE |
...
| Ghast Tear Necklace accessory, passive effects, server fireball ability, cooldown component, keybind, and recipe | COMPLETE |
...
- Do not remove the required Accessories dependency or the necklace slot/tag data files.
