...
Overworld ruined portal structure JSON files are overridden to use `ruined_portal_overhaul:portal_dungeon`. Their biome predicate now points at `#ruined_portal_overhaul:portal_dungeon_overworld`, a project tag backed by Fabric's conventional `#c:is_overworld` biome tag, so TerraBlender biome packs are included through convention tags instead of hardcoded vanilla ruined-portal tags. The matching `data/minecraft/worldgen/structure_set/ruined_portals.json` override now contains only the six overworld ruined portal variants, uses a minimum spacing grid of `32`, separation `20`, and the maximum 16-chunk data-side exclusion zone against `minecraft:villages`; `PortalDungeonStructure.findGenerationPoint(...)` then deterministically thins candidate chunks from the live `ModConfigManager.structureRarity()` value so pack authors can tune average rarity without a custom datapack.
...
- The prompt-level default rarity is 32 chunks. The vanilla ruined portal replacement structure set is data-driven at `data/minecraft/worldgen/structure_set/ruined_portals.json`, now using minimum `spacing` 32 and `separation` 20 so two radius-136 scars cannot overlap at the placement-grid level.
...
- `ModConfigManager.structureRarity()` is read inside `PortalDungeonStructure.findGenerationPoint(...)` for new candidate chunks. Values above 32 deterministically thin the minimum placement grid using the world seed and chunk position; already-generated chunks are not rewritten. Cloth Config clamps the option to 32-96 chunks because lower values would imply tighter placement than the data pack can safely provide for this large footprint.
